...
function pickRollupValue(row) {
  if (!row || typeof row !== "object") return 0;
  if (row[ROLLUP_COLUMN] != null) return toNumber(row[ROLLUP_COLUMN]);
...
function parseGroupedRows(response, dimension) {
  const totals = new Map();

  for (const row of response.summary ?? []) {
    const key = String(row[dimension] ?? "").trim();
    if (!key) continue;
    totals.set(key, (totals.get(key) ?? 0) + pickRollupValue(row));
  }
...
async function queryMetrics({ token, teamId, projectId, startTime, endTime, groupBy, limit }) {
  const body = {
    scope: {
      type: "project",
      ownerId: teamId,
    ...
    ...(groupBy?.length ? { groupBy, limit: limit ?? 20, orderBy: ROLLUP_COLUMN, orderDirection: "desc" } : {}),
  };
...
function extractTotalPageviews(response) {
  const summaryTotal = pickRollupValue(response.summary?.[0]);
  if (summaryTotal > 0) return summaryTotal;

  return (response.data ?? []).reduce((sum, row) => sum + pickRollupValue(row), 0);
}
...
export async function fetchVercelAnalytics() {
  ...
  try {
    const [totals, byCountry, byPath] = await Promise.all([
    ...
    return {
      ...
      pageviews: extractTotalPageviews(totals),
      countries,
      top_pages: topPages,
    };
  } catch (err) {
